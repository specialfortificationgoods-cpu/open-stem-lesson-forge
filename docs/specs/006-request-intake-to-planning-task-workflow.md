...
When retry limit is exceeded, the task transitions to `cancelled` with safe reason code `planning_abandoned_retry_limit`. The request transitions back to `requested` or remains `planning_open` according to deterministic API behavior in spec `008`; no work packets or artifacts are created.
